...
- Неправильное распределение ролей в команде: ассистент как друг, PM как ассистент
- Ошибка ограниченного майндсета: "у меня точно не получится"
- Как продать через оффер — разбор структуры
- Почему ваш сайт не продаёт: 3 типичные ошибки
- Как найти первого клиента без рекламного бюджета
- Что делать когда клиент говорит "дорого"
- Почему предприниматели боятся делегировать и как с этим работать
- Как выстроить воронку продаж за 1 день
- Ошибка в найме: берём "хорошего человека" вместо нужного специалиста
"""

GENERATE_PLAN_PROMPT = """ты — контент-стратег телеграм-канала Насти Базуевой @bazuevaconsalt.
канал для предпринимателей малого бизнеса — про продажи, маркетинг, операционку, найм, мышление.

сгенерируй 5 тем для учебных постов на неделю (пн-пт).
каждая тема — это конкретное наблюдение или кейс из практики консультанта.

требования к темам:
- конкретные, не абстрактные ("как продать через оффер" — хорошо, "о важности продаж" — плохо)
- из практики: ошибки клиентов, инсайты с консультаций, разборы реальных ситуаций
- полезные для предпринимателей малого бизнеса
- разные по тематике: продажи, маркетинг, операционка, найм, мышление
- в стиле прямолинейных наблюдений от практика
